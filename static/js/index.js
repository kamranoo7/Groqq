      ...
      <div class="${side}Msgbox ${side}_animation">
          <div class="msg ${side}-msg">
              ...
              <div class="msg-bubble">
                  ...
                  <div class="msg-text" data-id="${uniqueId}">${text}</div>
...
function changeVoiceSVG(transcript, id) {

  const chatext = document.querySelector(
    `[data-id=uniqueId_${transcriptID}]`
  );

  console.log(chatext,transcript, transcriptID, transcriptText, "chattext",);

  setTimeout(() => {
    chatext.innerHTML = `<div><p>${transcript}</p></div>`;
...
function getLevelResponse(
  ...
  fetch("/" + level, {
    ...
    .then((response) => response.json())
    .then((data) => {
      changeVoiceSVG(data.transcription)
      console.log(data.transcription,"user said something")
...
let currentAudio = null; // Variable to store reference to the current audio element
...
fetchDataAfterTwoSec(); // Start the interval to call fetchData every 100ms
